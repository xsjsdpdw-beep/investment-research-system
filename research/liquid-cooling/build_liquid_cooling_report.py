...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    /* Product Design readability pass: larger type, stronger contrast, clearer data surfaces. */
    body {{ font-size:15.5px; line-height:1.78; color:#162b39; }}
    .brand strong {{ font-size:18px; }} .brand small {{ font-size:11px; }}
    .rail-label {{ font-size:11px; }} .nav a {{ font-size:13px; line-height:1.55; padding:9px 8px; align-items:flex-start; }} .nav a span {{ flex:0 0 22px; font-size:11px; }}
    .hero-subtitle {{ color:#405661; font-size:18px; line-height:1.78; }} .hero-note {{ color:#4e636e; font-size:13.5px; line-height:1.78; }}
    .meta-chip {{ color:#4b616c; font-size:12px; padding:8px 11px; }} .stat-card span {{ font-size:11px; }} .stat-card small {{ color:#596e78; font-size:12px; }}
    .section-kicker {{ font-size:12px; }} .canvas h2, .article h2 {{ font-size:35px; }} .canvas-intro {{ color:#536873; font-size:16px; line-height:1.8; }}
    .segment-card p {{ font-size:13px; line-height:1.75; }} .company-tags span {{ font-size:11px; }} .priority-row span {{ font-size:11px; }} .priority-row strong {{ font-size:15px; }} .priority-row em {{ font-size:13px; }}
    .signal-card h3 {{ font-size:16px; }} .signal-card ul {{ font-size:13px; line-height:1.75; }} .board-head p {{ font-size:12px; }} .hardware-caption span {{ font-size:13px; }}
    .figure-card {{ padding:26px 25px 22px; border-radius:8px; box-shadow:0 16px 36px rgba(35,59,72,.10); }} .md-figure {{ margin:38px 0 52px; padding:28px 23px 22px; }}
    .figure-heading {{ margin-bottom:17px; }} .figure-heading span {{ font-size:11px; }} .figure-heading strong {{ font-size:21px; }}
    .figure-card svg text {{ font-size:14px; }} .figure-card svg .label {{ font-size:13px; }} .figure-card svg .axis, .figure-card svg .note {{ fill:#4d626c; font-size:12px; }} .figure-card svg .eyebrow {{ font-size:11px; }} .figure-card svg .value-label {{ font-size:12px; }} .figure-card svg .cell {{ font-size:12px; }}
    .figure-card svg .route-title, .figure-card svg .center-title {{ font-size:20px; }} .figure-card svg .center-sub {{ font-size:12px; }} .figure-card svg .step-number {{ font-size:12px; }} .figure-card svg .step-title {{ font-size:15px; }} .figure-card svg .check-label {{ font-size:11px; }}
    .figure-card svg .atlas-title {{ font-size:19px; }} .figure-card svg .atlas-note {{ fill:#526872; font-size:11px; }} .figure-card svg .column-title {{ font-size:18px; }} .figure-card svg .metric-title {{ font-size:18px; }} .figure-card svg .metric-sub, .figure-card svg .metric-companies {{ fill:#465d68; font-size:13px; }} .figure-card svg .node-title-dark {{ font-size:15px; }} .figure-card svg .node-sub-dark {{ fill:#536873; font-size:12px; }} .figure-card figcaption, .figure-card > figcaption {{ color:#536873; font-size:12px; line-height:1.75; }}
    .financial-plate {{ overflow-x:auto; }} .financial-dashboard {{ min-width:1000px; }} .figure-card svg .chart-title {{ font-size:18px; }} .figure-card svg .chart-unit, .figure-card svg .chart-axis, .figure-card svg .chart-legend {{ fill:#536873; font-size:12px; }} .figure-card svg .chart-label {{ fill:#304955; font-size:12px; }} .figure-card svg .chart-value {{ font-size:12px; }}
    .article h1 {{ font-size:46px; }} .article h2 {{ padding-top:64px; margin-top:56px; }} .article h3 {{ margin-top:38px; font-size:23px; }} .article p {{ color:#354d59; font-size:15.5px; line-height:1.85; }} .article ul, .article ol {{ color:#354d59; font-size:15px; line-height:1.85; }} .article li {{ margin:7px 0; }}
    .table-wrap {{ margin:18px 0 30px; border:1px solid #c8d6dc; border-radius:8px; background:#ffffff; box-shadow:0 10px 24px rgba(35,59,72,.06); }} table {{ min-width:760px; font-size:13.5px; }} th, td {{ padding:12px 14px; line-height:1.65; }} th {{ background:#e8f0f3; color:#1b3441; font-size:12px; letter-spacing:.02em; }} td {{ color:#374f5c; }} tbody tr:nth-child(even) td {{ background:#fbfcfc; }} tr:hover td {{ background:#eef6f8; }}
    :focus-visible {{ outline:3px solid rgba(46,118,201,.45); outline-offset:3px; }}
    @media (max-width:980px) {{ .nav a {{ font-size:13px; }} .hero-subtitle {{ font-size:17px; }} .article h2 {{ margin-top:45px; }} }}
    @media (max-width:600px) {{ .rail {{ padding:14px 17px 12px; }} .nav {{ gap:7px; padding-bottom:7px; }} .nav a {{ flex:0 0 170px; min-height:52px; white-space:normal; padding:8px 9px; border:1px solid #d7e2e7; background:#f8fbfb; border-radius:7px; font-size:12px; line-height:1.45; }} .nav a span {{ flex-basis:20px; }} .hero-subtitle {{ font-size:15.5px; line-height:1.75; }} .hero-note {{ font-size:13px; }} .figure-card {{ padding:20px 15px 17px; }} .figure-heading strong {{ font-size:19px; line-height:1.35; }} .figure-card svg {{ min-width:760px; }} .financial-dashboard {{ min-width:1000px; }} .article h2, .canvas h2 {{ font-size:29px; }} .article p {{ font-size:15px; }} .article ul, .article ol {{ font-size:14.5px; }} .table-wrap {{ margin-left:-1px; margin-right:-1px; }} }}
